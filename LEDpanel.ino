...
Adafruit_NeoMatrix matrix = Adafruit_NeoMatrix(32, 8,1, matrixHeight, 6,
...
void burninate() {
    ...
    for (int i = 0; i < matrix.height(); i++) {
        ...
        matrix.drawRGBBitmap(24, 0, spaceneedle_small, 6, 8);
        matrix.drawRGBBitmap(16, 0, godzilla_small, 7, 8);
        matrix.fillRect(0, 8-i, 32, i, 0xF800);
        matrix.show();
        delay(50);
    }
    ...
    for (int i = 0; i < matrix.height(); i++) {
        ...
        if (i % 3 == 0) {
            matrix.drawRGBBitmap(24, matrix.height()-i, spaceneedle_fire1, 6, 8);
        } else if (i % 3 == 1) {
            matrix.drawRGBBitmap(24, matrix.height()-i, spaceneedle_fire2, 6, 8);
        } else {
            matrix.drawRGBBitmap(24, matrix.height()-i, spaceneedle_fire3, 6, 8);
        }
        matrix.show();
        delay(50);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        int txtMsgLngth = theWord.length() * 6;
        // find the y position of the word
        int y = (matrix.height() - 8) / 2;
        // set the cursor 
        matrix.setCursor(x - txtMsgLngth / 2, 1);
...
//   matrix.setCursor(0, 0);
    ...
    while (x > -txtMsgLngth*7-1) {
        matrix.clear();
        matrix.fillScreen(0);
        matrix.setCursor(x, 0);
        matrix.setTextColor(0x03E0);
        matrix.print((txtMsg));
        if(--x < (-txtMsgLngth*7-1)) {
        x = matrix.width();
        if(++pass >= 3) pass = 0;
